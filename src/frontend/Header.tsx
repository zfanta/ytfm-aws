...
function SignInButton (): ReactElement {
  ...
  return (
    <>
      <Button id="sign-in" variant="outlined" onClick={() => setOpen(true)}>
        Sign in
      </Button>
      <Dialog
        open={open}
...
function YetAnotherButton (): ReactElement {
  const [location] = useLocation()
  ...
  switch (user) {
    ...
    case null:
      return <SignInButton/>
    default:
      return <ButtonsAfterSignIn email={user.email} photo={user.photos[0]}/>
  }
}
          ...
          <Container maxWidth="sm" style={{ padding: 0 }}>
            <Toolbar>
              <Typography variant="h6" style={{ flexGrow: 1 }}>
                <Link href={user !== null ? '/subscriptions' : '/'}>YTFM</Link>
              </Typography>
              <YetAnotherButton/>
          </Toolbar>
          </Container>
